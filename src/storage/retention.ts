...
export async function cacheControlForObject(env: Bindings, key: string, kind: ObjectKind): Promise<string> {
  ...
  while (true) {
    ...
    if (memberships.results.length > 0) hasMembership = true;
    if (memberships.results.length < 200) break;
    lastVersionId = memberships.results[memberships.results.length - 1].version_id;
  }

  if (!hasMembership) return ttlCacheControl(kind, UNCLASSIFIED_TTL_SECONDS);

  const policyRows = await env.DB.prepare("SELECT * FROM gc_policies ORDER BY id").all<PolicyRow>();
  // The first pass only establishes membership and bounds memory; retention
  // values must be evaluated with the current policy set in the second pass.
  lastVersionId = "";
  ...
  while (true) {
    ...
    for (const row of memberships.results) {
      const days = effectiveRetentionDays(row, policyRows.results, fallbackDays);
